...
export function scoreExtraction(actual: InvoiceDraft, expected: ExtractedInvoice): FidelityReport {
  ...
  for (const field of lineFields) fields[`lines.${field}`] = lines.length === expected.lines.length && lines.every((line, index) => same(line[field], expected.lines[index]?.[field])) ? 1 : 0
  return { fields, overall: Object.values(fields).reduce((sum, score) => sum + score, 0) / Object.keys(fields).length }
}
export const extractionFidelityScorer = createScorer<ExtractedInvoice, InvoiceDraft>({ id: 'extraction-fidelity', description: 'Deterministically scores invoice extraction fidelity before human correction.' })
