...
const evidence = (assessment: FinalAssessment, values: Partial<ApprovalEvidence>): ApprovalEvidence => ({ status: 'not_requested', reviewerId: null, decidedAt: null, invoiceDigest: digest(assessment), comment: null, ...values })
const result = (assessment: FinalAssessment, approval: ApprovalEvidence, executionStatus: Phase3Result['executionStatus']): Phase3Result => ({ ...assessment, executionStatus, approval, posting: null, postingError: null })
...
const post = createStep({
  id: 'post-approved-invoice', inputSchema: Phase3ResultSchema, outputSchema: Phase3ResultSchema,
  execute: async ({ inputData }) => {
    if (inputData.executionStatus !== 'ready_to_post') return inputData
    const adapter = activePhase2Runtime.provider.posting
    if (!adapter) return { ...inputData, executionStatus: 'posting_unavailable' as const, postingError: `Provider ${activePhase2Runtime.provider.id} does not support posting` }
    if (!inputData.vendor) return { ...inputData, executionStatus: 'posting_failed' as const, postingError: 'Validated vendor is missing' }
    try {
      const request = PostingRequestSchema.parse({ idempotencyKey: `ap-${inputData.approval.invoiceDigest}`, invoice: inputData.invoice, vendor: inputData.vendor, purchaseOrder: inputData.purchaseOrder, approval: inputData.approval })
      const posting = await adapter.postBill(request)
      await activePhase2Runtime.history.save({ id: posting.externalBillId, vendorId: inputData.vendor.id, invoiceNumber: inputData.invoice.invoiceNumber, invoiceDate: inputData.invoice.invoiceDate, currency: inputData.invoice.currency, totalMinor: inputData.invoice.totalMinor, channel: null })
      return { ...inputData, executionStatus: 'posted' as const, posting, postingError: null }
    } catch (error) { return { ...inputData, executionStatus: 'posting_failed' as const, postingError: error instanceof Error ? error.message : 'Unknown posting failure' } }
  },
})
