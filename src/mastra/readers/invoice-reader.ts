...
async function readLocalDocument(document: DocumentRef) {
  ...
  if (!Number.isSafeInteger(limit) || limit < 1)
    throw new Error("INVOICE_MAX_BYTES must be a positive integer");
  const file = await open(localPath, "r");
  let data: Buffer;
  try {
    if ((await file.stat()).size > limit)
      throw new Error(`Invoice exceeds INVOICE_MAX_BYTES (${limit})`);
  ...
  if (document.sha256 && document.sha256 !== sha256)
    throw new Error(`Invoice checksum mismatch for ${document.id}`);
...
if (readerType !== "fixture" && readerType !== "vision")
  throw new Error(`Unknown INVOICE_READER: ${readerType}`);
