...
export function detectMediaType(data: Uint8Array) {
  ...
  if (Buffer.from(data.subarray(0, 8)).equals(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]))) return 'image/png'
...
export async function prepareDocument(document: DocumentRef): Promise<DocumentRef> {
  if (document.localPath && !allowedMediaTypes.has(document.mimeType)) throw new Error(`Unsupported invoice media type: ${document.mimeType}`)
...
class FixtureInvoiceReader implements InvoiceReader {
  async read(document: DocumentRef) {
    ...
    if (document.localPath || document.mimeType !== fixture.document.mimeType || document.source !== fixture.document.source || document.sha256 !== fixture.document.sha256) throw new Error(`Document reference does not match fixture ${document.id}`)
