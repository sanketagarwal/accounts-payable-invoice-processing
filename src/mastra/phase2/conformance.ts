...
export interface FaultingPurchaseOrderCase { repository: PurchaseOrderRepository; poNumber: string }
export interface FaultingGoodsReceiptCase { repository: GoodsReceiptRepository; purchaseOrderId: string }
...
async function expectUnavailable(run: () => Promise<unknown>, label: string) {
  try { await run() } catch (error) { check(error instanceof ProviderUnavailableError, `${label} must throw ProviderUnavailableError`); return }
  throw new Error(`${label} must fail instead of returning an empty result`)
}
export async function runProviderConformance(provider: AccountingProvider, cases: ProviderConformanceCases): Promise<ConformanceReport> {
  ...
  if (provider.vendors) {
    ...
    check(found.length > 0, 'Known vendor was not found'); found.forEach(value => VendorRecordSchema.parse(value))
    check((await provider.vendors.find(cases.vendors!.missing)).length === 0, 'Missing vendor must return []')
    if (cases.vendors!.transportFailure) await expectUnavailable(() => cases.vendors!.transportFailure!.repository.find(cases.vendors!.transportFailure!.lookup), 'Vendor transport failure')
  ...
  if (provider.purchaseOrders) {
    ...
    check(found.length > 0, 'Known purchase order was not found'); found.forEach(value => PurchaseOrderSchema.parse(value))
    check((await provider.purchaseOrders.findByNumber(cases.purchaseOrders!.missing)).length === 0, 'Missing purchase order must return []')
    if (cases.purchaseOrders!.transportFailure) await expectUnavailable(() => cases.purchaseOrders!.transportFailure!.repository.findByNumber(cases.purchaseOrders!.transportFailure!.poNumber), 'Purchase-order transport failure')
  ...
  if (provider.goodsReceipts) {
    ...
    check(found.length > 0, 'Known goods receipt was not found'); found.forEach(value => GoodsReceiptSchema.parse(value))
    check((await provider.goodsReceipts.findByPurchaseOrderId(cases.goodsReceipts!.missing)).length === 0, 'Missing goods receipt must return []')
    if (cases.goodsReceipts!.transportFailure) await expectUnavailable(() => cases.goodsReceipts!.transportFailure!.repository.findByPurchaseOrderId(cases.goodsReceipts!.transportFailure!.purchaseOrderId), 'Goods-receipt transport failure')
  ...
  if (provider.sanctions) {
    ...
    check(vendor, 'Known vendor is required to test sanctions'); SanctionsResultSchema.parse(await provider.sanctions.screen(vendor!))
