import { FixturePolicyProvider, FixtureSanctionsScreener, InMemoryInvoiceHistoryRepository } from './adapters/fixture.ts'
import type { InvoiceHistoryRepository, PolicyProvider, SanctionsScreener, VendorStatusRestrictionSource } from './ports.ts'
...
export interface Phase2Runtime {
  provider: AccountingProvider; history: InvoiceHistoryRepository; policy: PolicyProvider; sanctions: SanctionsScreener;
  sanctionsIsFallback: boolean; statusRestrictions?: VendorStatusRestrictionSource; seedHistory(): Promise<void>;
}
export function createPhase2Runtime(options: {
  provider?: AccountingProvider; providerId?: string; history?: InvoiceHistoryRepository; policy?: PolicyProvider;
  sanctionsFallback?: SanctionsScreener; statusRestrictions?: VendorStatusRestrictionSource;
} = {}): Phase2Runtime {
  const provider = options.provider ?? providerRegistry.create(options.providerId ?? process.env.ACCOUNTING_PROVIDER ?? 'fixture')
...
export const runtimeSources = (runtime: Phase2Runtime) => ({
  vendors: sourceId(runtime.provider, 'vendors'), purchaseOrders: sourceId(runtime.provider, 'purchaseOrders'),
  goodsReceipts: sourceId(runtime.provider, 'goodsReceipts'), sanctions: runtime.sanctionsIsFallback ? 'standalone-sanctions' : sourceId(runtime.provider, 'sanctions'),
