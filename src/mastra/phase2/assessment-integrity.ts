...
const signingKey = () => {
  ...
  const knownValues = new Set([
    ...
    "local-development-assessment-key",
  ]);
  if (key && !knownValues.has(key) && key.length >= 32) return key;
  if (
    process.env.NODE_ENV === "production" ||
    process.env.QBO_MCP_ENABLE_POSTING?.trim().toLowerCase() === "true"
  )
    throw new Error(
      "A server-only AP_ASSESSMENT_SIGNING_KEY is required for production or QuickBooks posting",
    );
...
export const verifyAssessment = (assessment: Record<string, unknown>) => {
  const actual =
    typeof assessment.assessmentSignature === "string" ? assessment.assessmentSignature : "";
