...
class CrosswalkPostingAdapter implements PostingAdapter {
  ...
  async postBill(input: Parameters<PostingAdapter["postBill"]>[0]) {
    ...
    if (input.purchaseOrder && !poId)
      throw new ReferenceCrosswalkError("purchaseOrder", input.purchaseOrder.id);
...
export function makeCompositeProvider(config: CompositeProviderConfig): AccountingProvider {
  ...
  if (vendor && po && vendor.id !== po.id) {
    ...
    if (!from || !to)
      throw new Error("Composite vendor/PO sources must declare identity namespaces");
    if (!shared(from, to, config.identity) && !config.identity?.crosswalk?.mapVendorId)
      throw new Error(
        "Composite vendor/PO sources require a shared namespace or vendor ID crosswalk",
      );
    if (!shared(from, to, config.identity) && purchaseOrders)
      purchaseOrders = new CrosswalkPurchaseOrderRepository(
        purchaseOrders,
        from,
        to,
        config.identity!,
  ...
  if (po && receipt && po.id !== receipt.id) {
    ...
    if (!from || !to)
      throw new Error("Composite PO/receipt sources must declare identity namespaces");
    if (!shared(from, to, config.identity) && !config.identity?.crosswalk?.mapPurchaseOrderId)
      throw new Error(
        "Composite PO/receipt sources require a shared namespace or purchase-order ID crosswalk",
      );
  ...
  if (vendor && history && vendor.id !== history.id) {
    const from = history.identityNamespaces?.billHistoryVendorIds,
      to = vendor.identityNamespaces?.vendors;
    if (!from || !to)
      throw new Error("Composite vendor/history sources must declare identity namespaces");
    if (!shared(from, to, config.identity) && !config.identity?.crosswalk?.mapVendorId)
      throw new Error(
        "Composite vendor/history sources require a shared namespace or vendor ID crosswalk",
      );
  ...
  if (posting) {
    ...
    if (po && (!poFrom || !poTo))
      throw new Error("Composite PO/posting sources must declare identity namespaces");
    const vendorPair = shared(vendorFrom, vendorTo, config.identity)
      ? undefined
      : ([vendorFrom, vendorTo] as [string, string]);
    const poPair =
      po && !shared(poFrom!, poTo!, config.identity)
        ? ([poFrom!, poTo!] as [string, string])
        : undefined;
    if (vendorPair && !config.identity?.crosswalk?.mapVendorId)
      throw new Error(
        "Composite vendor/posting sources require a shared namespace or vendor ID crosswalk",
      );
    if (poPair && !config.identity?.crosswalk?.mapPurchaseOrderId)
      throw new Error(
        "Composite PO/posting sources require a shared namespace or purchase-order ID crosswalk",
      );
    if (vendorPair || poPair)
      posting = new CrosswalkPostingAdapter(posting, vendorPair, poPair, config.identity);
  }
