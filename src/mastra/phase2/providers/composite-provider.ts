import { ReferenceCrosswalkError, type GoodsReceiptRepository, type PostingAdapter, type PurchaseOrderRepository } from '../ports.ts'
...
export interface CompositeProviderConfig {
  ...
  sanctions?: AccountingProvider; billHistory?: AccountingProvider; posting?: AccountingProvider; identity?: CompositeIdentityConfig;
}
...
class CrosswalkPostingAdapter implements PostingAdapter {
  constructor(private readonly delegate: PostingAdapter, private readonly vendorNamespaces?: [string, string], private readonly poNamespaces?: [string, string], private readonly config?: CompositeIdentityConfig) {}
  async postBill(input: Parameters<PostingAdapter['postBill']>[0]) {
    const vendorId = this.vendorNamespaces ? await this.config?.crosswalk?.mapVendorId?.({ id: input.vendor.id, fromNamespace: this.vendorNamespaces[0], toNamespace: this.vendorNamespaces[1] }) : input.vendor.id
    const poId = input.purchaseOrder && this.poNamespaces ? await this.config?.crosswalk?.mapPurchaseOrderId?.({ id: input.purchaseOrder.id, fromNamespace: this.poNamespaces[0], toNamespace: this.poNamespaces[1] }) : input.purchaseOrder?.id
    ...
    return this.delegate.postBill({ ...input, vendor: { ...input.vendor, id: vendorId }, purchaseOrder: input.purchaseOrder ? { ...input.purchaseOrder, id: poId!, vendorId } : null })
  }
}
export function makeCompositeProvider(config: CompositeProviderConfig): AccountingProvider {
  const vendor = config.vendors, po = config.purchaseOrders, receipt = config.goodsReceipts, sanctions = config.sanctions, history = config.billHistory, postingSource = config.posting
  let purchaseOrders = po?.purchaseOrders, goodsReceipts = receipt?.goodsReceipts, billHistorySeed = history?.billHistorySeed, posting = postingSource?.posting
  ...
  if (posting) {
    const vendorFrom = vendor?.identityNamespaces?.vendors, vendorTo = postingSource?.identityNamespaces?.postingVendorIds
    const poFrom = po?.identityNamespaces?.purchaseOrders, poTo = postingSource?.identityNamespaces?.postingPurchaseOrders
    ...
    const vendorPair = shared(vendorFrom, vendorTo, config.identity) ? undefined : [vendorFrom, vendorTo] as [string, string]
    const poPair = po && !shared(poFrom!, poTo!, config.identity) ? [poFrom!, poTo!] as [string, string] : undefined
    if (vendorPair && !config.identity?.crosswalk?.mapVendorId) throw new Error('Composite vendor/posting sources require a shared namespace or vendor ID crosswalk')
    if (poPair && !config.identity?.crosswalk?.mapPurchaseOrderId) throw new Error('Composite PO/posting sources require a shared namespace or purchase-order ID crosswalk')
  ...
  return assertProvider({
    ...
    capabilities: {
      ...
      invoiceChannel: history?.capabilities.invoiceChannel ?? false, posting: Boolean(posting),
    },
    vendors: vendor?.vendors, purchaseOrders, goodsReceipts, sanctions: sanctions?.sanctions, billHistorySeed, posting,
    sources: {
    ...
    identityNamespaces: { vendors: vendor?.identityNamespaces?.vendors, purchaseOrders: po?.identityNamespaces?.purchaseOrders, purchaseOrderVendorIds: vendor?.identityNamespaces?.vendors, goodsReceipts: po?.identityNamespaces?.purchaseOrders, billHistoryVendorIds: vendor?.identityNamespaces?.vendors, postingVendorIds: vendor?.identityNamespaces?.vendors, postingPurchaseOrders: po?.identityNamespaces?.purchaseOrders },
  })
}
