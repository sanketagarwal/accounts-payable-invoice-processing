...
export function makeQuickBooksProvider(client?: QboClient): AccountingProvider {
  const resolved = client ?? (() => {
    const realmId = process.env.QBO_REALM_ID, accessToken = process.env.QBO_ACCESS_TOKEN
  ...
  const adapter = new QuickBooksAdapter(resolved)
  return assertProvider({
    id: 'quickbooks', displayName: 'QuickBooks Online',
    ...
    vendors: adapter, purchaseOrders: adapter, billHistorySeed: () => adapter.billHistorySeed(), identityNamespaces: { vendors: 'quickbooks', purchaseOrders: 'quickbooks' },
  })
}
