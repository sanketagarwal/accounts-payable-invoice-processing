...
const entries = (invoice: Phase2Invoice) => invoice.confidence.map(item => ({ ...item, field: canonicalConfidenceField(item.field) }))
...
export const requiredConfidenceFields = (invoice: Phase2Invoice) => [
  'invoiceNumber', 'vendorName', 'poNumber', 'invoiceDate', 'currency', 'subtotal', 'tax', 'total',
  ...invoice.lines.flatMap((line, index) => [
    `lines.${index}.description`, `lines.${index}.qty`, `lines.${index}.unitPrice`,
...
export const hasLowConfidence = (invoice: Phase2Invoice, fields: string[], threshold: number) => {
  ...
  return fields.some(field => {
    const canonical = canonicalConfidenceField(field)
    if (canonical === 'lines') return values.some(item => (item.field === 'lines' || item.field.startsWith('lines.')) && item.confidence < threshold)
    if (lineLeaf.test(field)) return values.some(item => item.field.endsWith(`.${field}`) && item.confidence < threshold)
