...
type QboVendor = { Id?: string; DisplayName?: string; Active?: boolean; TaxIdentifier?: string }
type QboLine = { Amount?: number; Description?: string; ItemBasedExpenseLineDetail?: { ItemRef?: Ref; Qty?: number; UnitPrice?: number } }
type QboPurchaseOrder = { Id?: string; DocNumber?: string; VendorRef?: Ref; CurrencyRef?: Ref; TotalAmt?: number; Line?: QboLine[] }
type QboBill = { Id?: string; DocNumber?: string; VendorRef?: Ref; CurrencyRef?: Ref; TotalAmt?: number; TxnDate?: string }
...
export class HttpQboClient implements QboClient {
  ...
  async query<T>(entity: string, query: string): Promise<T[]> {
    try {
      const response = await fetch(`${this.baseUrl}/v3/company/${this.realmId}/query?query=${encodeURIComponent(query)}&minorversion=75`, { headers: { Authorization: `Bearer ${this.accessToken}`, Accept: 'application/json' } })
      if (!response.ok) throw new Error(`HTTP ${response.status}`)
      const body = await response.json() as { QueryResponse?: Record<string, T[]> }
      return body.QueryResponse?.[entity] ?? []
    } catch (error) { throw new QboUnavailableError(`query ${entity}`, error) }
  }
}
export class QuickBooksAdapter implements VendorRepository, PurchaseOrderRepository {
  ...
  async findByNumber(poNumber: string) {
    const rows = await this.client.query<QboPurchaseOrder>('PurchaseOrder', `select * from PurchaseOrder where DocNumber = '${quote(poNumber)}'`)
    return rows.map(row => {
      const currency = row.CurrencyRef?.value ?? 'USD'
      return PurchaseOrderSchema.parse({
        id: required(row.Id, 'PurchaseOrder.Id'), poNumber: required(row.DocNumber, 'PurchaseOrder.DocNumber'), vendorId: required(row.VendorRef?.value, 'PurchaseOrder.VendorRef'),
        currency, totalMinor: toMinorUnits(row.TotalAmt ?? 0, currency), lines: (row.Line ?? []).filter(line => line.ItemBasedExpenseLineDetail).map(line => {
          const detail = line.ItemBasedExpenseLineDetail!, qty = detail.Qty ?? 0, amount = line.Amount ?? 0
          return { sku: detail.ItemRef?.value ?? null, description: line.Description ?? detail.ItemRef?.name ?? '', qty, unitPriceMinor: toMinorUnits(detail.UnitPrice ?? (qty ? amount / qty : 0), currency), lineTotalMinor: toMinorUnits(amount, currency) }
        }),
      })
    })
  }
