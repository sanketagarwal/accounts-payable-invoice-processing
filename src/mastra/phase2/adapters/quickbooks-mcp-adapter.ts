...
export class QuickBooksMcpAdapter implements VendorRepository, PurchaseOrderRepository, PostingAdapter {
  ...
  constructor(private readonly client: McpToolClient, private readonly poLimit = 1000, private readonly postingConfig?: QuickBooksMcpPostingConfig) { if (!Number.isInteger(poLimit) || poLimit < 1 || poLimit > 1000) throw new Error('QuickBooks MCP PO limit must be an integer from 1 to 1000') }
  async verifyTools() {
    const tools = await this.client.listToolNames(), expected = [...requiredTools, ...(this.postingConfig ? [postingTool] : [])], missing = expected.filter(tool => !tools.has(tool))
    const mutations = [...tools].filter(tool => this.postingConfig ? /^(update|delete)[_-]/.test(tool) : /^(create|update|delete)[_-]/.test(tool))
    if (missing.length) throw new Error(`QuickBooks MCP is missing required tools: ${missing.join(', ')}`)
    if (mutations.length) throw new Error(`QuickBooks MCP provider refuses unsupported mutation tools: ${mutations.join(', ')}`)
  }
  ...
  private async withPostingLock(input: PostingRequest) {
    ...
    const lock = resolve(root, createHash('sha256').update(input.idempotencyKey).digest('hex'))
  ...
  private async post(input: PostingRequest) {
    ...
    if (tax && !config.taxAccountId) throw new PostingConflictError('QBO_MCP_TAX_ACCOUNT_ID is required to post an invoice with tax')
    const amounts = input.invoice.lines.map(line => line.lineTotalMinor ?? new Decimal(line.unitPriceMinor).mul(line.qty).toDecimalPlaces(0, Decimal.ROUND_HALF_UP).toNumber())
    const expectedSubtotal = input.invoice.subtotalMinor ?? input.invoice.totalMinor - tax
    if (amounts.reduce((sum, amount) => sum + amount, 0) !== expectedSubtotal) throw new PostingConflictError('Invoice lines do not reconcile to the posting subtotal')
    const line = input.invoice.lines.map((item, index) => ({
      Amount: major(amounts[index]!, input.invoice.currency), DetailType: 'AccountBasedExpenseLineDetail', Description: item.description,
      AccountBasedExpenseLineDetail: { AccountRef: { value: config.expenseAccountId } },
    }))
    if (tax) line.push({ Amount: major(tax, input.invoice.currency), DetailType: 'AccountBasedExpenseLineDetail', Description: 'Invoice tax', AccountBasedExpenseLineDetail: { AccountRef: { value: config.taxAccountId! } } })
    const bill = {
    ...
    return PostingReceiptSchema.parse({ status: 'posted', providerId: 'quickbooks-mcp', externalBillId: created.Id, postedAt: new Date().toISOString(), idempotencyKey: input.idempotencyKey })
  }
