  ...
  return async (state: AssessmentState) => {
    ...
    try {
      ...
      if (error instanceof ReferenceCrosswalkError) {
        state.decisions.push({ step: 'dedup', outcome: 'review', reviewType: 'identity_crosswalk_missing', reasons: [{ code: 'IDENTITY_CROSSWALK_MISSING', message: error.message, evidence: { entity: error.entity, id: error.id } }], signals: [], adaptations, sources: { billHistory: sources.billHistory } }); return AssessmentStateSchema.parse(state)
      }
