  ...
  return async (state: AssessmentState) => {
    ...
    try {
      ...
      if (!state.invoice.poNumber) {
        state.decisions.push({ step: 'match', outcome: 'review', reviewType: 'missing_po', reasons: [{ code: 'PO_NUMBER_MISSING', message: 'Invoice has no printed PO number' }], signals: [], adaptations, sources: {} }); return AssessmentStateSchema.parse(state)
      }
      const orders = await provider.purchaseOrders!.findByNumber(state.invoice.poNumber)
      if (orders.length !== 1) {
        state.decisions.push({ step: 'match', outcome: 'review', reviewType: orders.length ? 'ambiguous_po' : 'po_not_found', reasons: [{ code: orders.length ? 'PO_AMBIGUOUS' : 'PO_NOT_FOUND', message: orders.length ? 'Multiple purchase orders match' : 'Purchase order was not found' }], signals: [], adaptations, sources: { purchaseOrders: sources.purchaseOrders } }); return AssessmentStateSchema.parse(state)
      }
      ...
      if (!provider.goodsReceipts) {
        ...
        state.decisions.push({ step: 'match', outcome: 'pass', reviewType: null, reasons: [{ code: 'TWO_WAY_MATCH', message: 'Invoice and purchase order match; source supplies no goods receipts', evidence: { matchMode: 'two_way' } }], signals: [], adaptations, sources: { purchaseOrders: sources.purchaseOrders } }); return AssessmentStateSchema.parse(state)
      }
      state.matchMode = 'three_way'; state.receipts = await provider.goodsReceipts.findByPurchaseOrderId(po.id)
      const received = new Map<string, number>()
      for (const receipt of state.receipts) receipt.lines.forEach((line, index) => { const key = line.sku ?? `line:${index}`; received.set(key, (received.get(key) ?? 0) + line.qty) })
      const receiptMismatch = !state.receipts.length || state.invoice.lines.some((line, index) => (received.get(line.sku ?? `line:${index}`) ?? 0) < line.qty)
