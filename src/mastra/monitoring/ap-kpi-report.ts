...
export function buildApKpiReport(rows: ApKpiEvent[]) {
  ...
  const latest = ordered.map(
    (events) =>
      [...events].reverse().find((row) => row.approvalState !== "resume_failed") ?? events.at(-1)!,
  );
  ...
  const completed = latest.filter(
    (row) => !isPending(row) && row.approvalState !== "resume_failed",
  );
  const isPosted = (row: ApKpiEvent) =>
    row.postingStatus === "posted" || row.postingStatus === "already_posted";
  const posted = completed.filter(isPosted);
  const straightThrough = completed.filter(
    (row) => row.disposition === "auto_post" && isPosted(row),
  );
  const approvalTimes = ordered.flatMap((events) => {
    ...
    return pending && resolved
      ? [Date.parse(resolved.recordedAt) - Date.parse(pending.recordedAt)]
      : [];
  });
  ...
  return {
    runs: latest.length,
    straightThroughProcessingRate: completed.length
      ? straightThrough.length / completed.length
      : null,
    ...
    approvalTimeMs: approvalTimes.length
      ...
      : null,
    integrationFailures: ordered.filter((events) => events.some((row) => row.integrationFailure))
      .length,
