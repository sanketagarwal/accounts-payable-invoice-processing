...
export const apInvoiceWorkflow = createWorkflow({
  id: 'ap-invoice-workflow', inputSchema: DocumentRefSchema, outputSchema: Phase3ResultSchema,
  requestContextSchema: ReviewerContextSchema,
  options: { shouldPersistSnapshot: () => true },
}).then(invoiceReaderWorkflow).then(apDecisionWorkflow).then(apExecutionWorkflow).commit()
