...
import {
  ...
  invoiceReader,
  prepareDocument,
} from "../mastra/readers/invoice-reader.ts";
import {
  extractionFidelityScorer,
  scoreExtraction,
} from "../mastra/scorers/extraction-fidelity.ts";
...
assert.equal(
  validateExtraction({ ...cleanExtraction, vendorTaxId: undefined }).extracted?.vendorTaxId,
  null,
);
assert.ok(
...
assert.ok(
  validateExtraction({ ...cleanExtraction, subtotal: 99 }).issues.includes(
...
assert.equal(
  validateExtraction({ ...cleanExtraction, total: Number.POSITIVE_INFINITY }).extracted,
  null,
);
assert.equal(
  validateExtraction({ ...cleanExtraction, currency: "XAU" }).extracted?.currency,
  "XAU",
);
assert.equal(
  validateExtraction({ ...cleanExtraction, currency: "XCG" }).extracted?.currency,
  "XCG",
);
assert.equal(
  validateExtraction({ ...cleanExtraction, currency: "XAD" }).extracted?.currency,
  "XAD",
);
assert.ok(
...
assert.ok(
  validateExtraction({ ...cleanExtraction, total: 108.004 }).issues.includes(
    "total exceeds USD minor-unit precision",
  ),
);
assert.ok(
  validateExtraction({
...
assert.ok(
  validateExtraction({ ...cleanExtraction, total: 108.01 }).issues.includes(
...
assert.equal(
  scoreExtraction(
    { ...cleanExtraction, invoiceNumber: "001" },
    { ...cleanExtraction, invoiceNumber: "1" },
  ).fields.invoiceNumber,
  0,
);
assert.equal(
  scoreExtraction(
    { ...cleanExtraction, lines: [{ ...cleanExtraction.lines[0]!, qty: 9 }] },
    cleanExtraction,
  ).fields["lines.qty"],
  0,
);
...
try {
  ...
  await assert.rejects(
    prepareDocument({ id: "source-mismatch", mimeType: "image/png", source: "PDF" }),
    /conflicts/,
  );
...
if (sourceResult.status === "success")
  assert.equal(sourceResult.result.extractedResult.source, "PDF");
...
if (secondReview.status === "suspended")
  assert.equal(secondReview.suspendPayload["verify-invoice"].draft.total, 61);
...
assert.equal(
  (await new QuickBooksAdapter(noDocNumberClient).billHistorySeed())[0]!.invoiceNumber,
  null,
);
...
assert.equal(
  (await makeQuickBooksMcpProvider(noDocNumberMcp).billHistorySeed!())[0]!.invoiceNumber,
  null,
);
assert.deepEqual(
...
const writableMcp: McpToolClient = {
  listToolNames: async () =>
    new Set(["search_vendors", "search_purchase_orders", "search_bills", "create-bill"]),
...
await assert.rejects(
  recoveringProvider.vendors!.find({ name: "Acme Supplies" }),
  ProviderUnavailableError,
);
assert.equal(
  (await recoveringProvider.vendors!.find({ name: "Acme Supplies" }))[0]!.id,
  "qbo_vendor_acme",
);
...
await assert.rejects(
  new QuickBooksMcpAdapter(truncatedMcp, 1).findByNumber("PO-MISSING"),
  ProviderUnavailableError,
);
...
assert.ok(
  qboState.decisions.at(-1)!.adaptations.some((a) => a.code === "GOODS_RECEIPTS_UNAVAILABLE"),
);
...
assert.deepEqual(missingLineConfidenceState.decisions[0]!.reasons[0]!.evidence?.missingConfidence, [
  "lines.0.qty",
]);
...
assert.deepEqual(
  conflictingLineConfidenceState.decisions[0]!.reasons[0]!.evidence?.uncertainFields,
  ["lines.0.qty"],
);
...
assert.ok(
  quantityVarianceState.decisions
    .at(-1)!
    .reasons.some((reason) => reason.code === "QUANTITY_VARIANCE"),
);
...
assert.ok(
  lineMismatchState.decisions.at(-1)!.reasons.some((reason) => reason.code === "PRICE_VARIANCE"),
);
assert.ok(
  lineMismatchState.decisions.at(-1)!.reasons.some((reason) => reason.code === "QUANTITY_VARIANCE"),
);
...
assert.equal(
  (await makePolicyRouting(fixtureRuntime)(identityMismatchState)).disposition,
  "review",
);
...
for (const vendor of [
  ...
  assert.ok(
    priorityState.decisions[0]!.reasons.some((reason) => reason.code === "VENDOR_TAX_ID_MISMATCH"),
  );
...
if (approvalStart.status === "suspended") {
  ...
  assert.ok(
    approvalSummary.reasonDetails.some((reason) => reason.code === "APPROVAL_THRESHOLD_EXCEEDED"),
  );
}
...
if (rejectedExecution.status === "success")
  assert.equal(rejectedExecution.result.executionStatus, "rejected");
...
try {
  ...
  process.env.QBO_MCP_ENABLE_POSTING = "true";
  assert.throws(
    () => signAssessment({ disposition: "auto_post" }),
    /server-only AP_ASSESSMENT_SIGNING_KEY/,
  );
...
const unavailableState = await makeVendorValidation(
  createPhase2Runtime({ provider: unavailableProvider }),
)(normalized);
