...
const scores: number[] = [], imperfect: string[] = [], regressions: string[] = []
for (const fixture of invoiceFixtures) {
  ...
  if (score.score < fixture.minimumFidelity) regressions.push(`${fixture.document.id}:${score.score.toFixed(3)}<${fixture.minimumFidelity}`)
  console.log(`${fixture.document.id}: ${score.score.toFixed(3)} ${score.reason}`)
}
console.log(`mean=${(scores.reduce((sum, score) => sum + score, 0) / scores.length).toFixed(3)} imperfect=${imperfect.join(',') || 'none'} regressions=${regressions.join(',') || 'none'}`)
